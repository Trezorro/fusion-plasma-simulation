...
def predict(args, modelJoint):
    ...
    for j in np.arange(length):
        ...
        elif no_input_channels == 4:
            scalars = np.asarray([vals.FIR, vals.PD, vals.TDAI, vals.DML]).swapaxes(0, 1)
    ...
    out_df.to_csv(path_or_buf=os.path.join(shots_dir, machine_id + '_'  + str(shot) + '_ConvLSTM_det.csv'.format(train_machine, machine_id)), index=False)
