...
**In-run summary** (`summarize_rollouts()` in `src/rollout.py`, logged as `rollout/final/*` and written to the `{rollout.cache_name}.json` sidecar): rollout counts, per-channel absolute moment errors `abs_mean_err/{ch}` = |mean(gen) - mean(real)| and `abs_std_err/{ch}` (pooled over all rollouts, normalized units), overall surrogate label agreement and per-mode dice over the generated spans, and the fraction of generated samples outside the [0,1] data range (feedback drift). Deliberately small; it is a sanity signal, not the analysis.

**Horizon-resolved analysis** (`src/plotters/rollout_horizon.py`, from the cache; produced both in-run when `rollout.analysis` is true and post-hoc by `eval_notebooks/rollout_analysis.py`). The single-window metrics cannot see feedback drift (the model consuming its own output); these curves measure quality as a function of the autoregressive depth k, where window k covers the generated samples `[k*step, k*step + seq_length)`, i.e. k counts how many windows of its own output the model has been fed.

*Dimensions.* The atomic unit is one (model, rollout, k) triple, where a model is one rollout cache (the notebook takes a `MODELS` list of caches and overlays them) and a rollout is one (shot, start fraction, sample index) triple. Per (rollout, k) and always against the real trace over the exact same time span (both normalized [0,1]):

| Column | Definition |
|---|---|
| `abs_mean_err/{ch}` | absolute mean error \|mean(gen_ch) - mean(real_ch)\| over the window; horizon extension of the window moment errors (section 1): mean drift or collapse-to-the-mean of the feedback loop shows up here first |
| `abs_std_err/{ch}` | \|std(gen_ch) - std(real_ch)\|, same idea for the spread |
| `label_agreement` | fraction of the window's samples where FNOLSTM(gen) == FNOLSTM(real) (unshifted 0=L,1=D,2=H); a horizon-resolved cousin of the dice score |
| `elm_peaks_gen`, `elm_peaks_real` | count of `scipy.signal.find_peaks` on the window's normalized PD with `prominence = evaluation.peaks.elm_pd_prominence`, the same definition as the window PeakMetric's `PD large peaks` channel; compared as two curves: does the model keep producing ELM-scale bursts at the real rate as k grows? |

*How to read the figures* (`output/pdfplots/{run_name}/rollout_analysis/{WxH}/{metric}.pdf` in-run, or `output/pdfplots/rollout_analysis/{WxH}/{metric}.pdf` post-hoc; several sizes): every figure is faceted into ONE PANEL PER START FRACTION. This is deliberate and load-bearing: at the same depth k, a rollout started at 10% of the shot is in ramp-up while one started at 75% is deep in H-mode; pooling them would confound autoregressive depth with shot phase, so different starting points are never averaged together. Within a panel the line is the MEDIAN over that start fraction's rollouts reaching depth k (one line per model), and the shaded band is the IQR (25th-75th percentile) over the same population: spread across rollouts, not uncertainty of the median. The grey step line on the right axis is n(k), the number of rollouts still running at depth k. Mind the survivor effect it exposes: deep k is only reached by long shots, so the population changes along the x axis even within a panel, and a bend at large k can be a population change rather than model behaviour. The per-(rollout, k) dataframe keeps `model`, `shot`, `start_frac`, and `sample_idx` columns, so everything can be re-sliced further (per shot, per single rollout). A compact LaTeX table (rows = start fraction x selected depths, main model) and the full per-(model, start fraction, k) CSV are exported alongside. A question-oriented guide to these figures lives in [plots.md](plots.md).

The window metrics (PeakMetric, ModeTransitionMetric, dice) are deliberately NOT applied to rollout windows: they condition on the true mode of a *real* history window, and after the first window a rollout's history is generated, so the bucketing semantics would silently change. Rollout surrogate labels come from the same FNOLSTM path as the window labels (a classifier pass over the full trace, LSTM state carried) and use the same unshifted `0=L, 1=D, 2=H` convention in the cache. Labeling is batched per `(shot, start_i)` group rather than per rollout (`src.rollout.label_rollout_group`): every `sample_idx` at the same start point shares the real trace and length, so one FNOLSTM call handles the whole group. This keeps labeling cost independent of `rollout.n_samples`, which otherwise would dominate wall time at high sample counts since it isn't batched across `max_batch` like generation is.
...
| Does quality survive the model feeding on its own output (drift, collapse, dying transients)? | Rollout horizon curves (section 7) |
| Does the rollout stay in the right confinement mode over long horizons? | Rollout label agreement (section 7) |
