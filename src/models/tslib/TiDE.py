...
class Model(nn.Module):
    ...
    def __init__(self, configs, bias=True, feature_encode_dim=2):
        ...
        # VENDOR EDIT: gate the inline non-stationary normalization (upstream has no flag).
        ...
        # VENDOR EDIT: free the dynamic-covariate ("mark") width from the calendar-frequency map.
        # Upstream fixes feature_dim to freq_map[freq] (calendar features). PlasmaFlow routes real
        # control covariates C through this slot, so its width is the number of C channels. Falls
        # back to the calendar map when no covariates are configured (c_channels == 0 / unset).
        self.feature_dim = getattr(configs, 'c_channels', 0) or freq_map[self.freq]
